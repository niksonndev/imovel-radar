            ...
            <div className="overflow-hidden rounded-[1.6rem] bg-background">
              ...
              <div className="flex flex-col gap-2 px-4 py-4">
                {MOCK_ALERTS.map((alert) => (
                  <div
                    key={alert.title}
                    className="max-w-[90%] self-start rounded-2xl rounded-bl-sm bg-muted px-3.5 py-2.5"
                  >
                    <p className="text-[10px] font-mono uppercase tracking-wider text-primary-on-surface">
                      {alert.kind === "venda" ? "Venda" : "Aluguel"}
                    </p>
                    <p className="mt-0.5 text-sm font-medium text-white">
                      🏠 {alert.title}
                    </p>
                    <p className="mt-0.5 text-xs text-white/60">{alert.detail}</p>
                    <p className="mt-1 text-right font-mono text-[10px] text-white/45">
                      {alert.time}
                    </p>
                  </div>
                ))}
